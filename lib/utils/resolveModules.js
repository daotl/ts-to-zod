...
const moduleToPrefix = (declarationNames) => (context) => (sourceFile) => {
    const prefixInterfacesAndTypes = (moduleName) => (node) => {
        ...
        if (typescript_1.default.isInterfaceDeclaration(node)) {
            return typescript_1.factory.updateInterfaceDeclaration(node, node.decorators, node.modifiers, typescript_1.factory.createIdentifier((0, case_1.pascal)(moduleName) + (0, case_1.pascal)(node.name.text)), node.typeParameters, typescript_1.default.visitNodes(node.heritageClauses, prefixInterfacesAndTypes(moduleName)), typescript_1.default.visitNodes(node.members, prefixInterfacesAndTypes(moduleName)));
        }
