...
describe("generateZodSchema", () => {
    it("should generate a string schema", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const myHeroNameSchema = z.string();"`);
    });
    it("should generate a number schema", () => {
        const source = `export type MyHeroAge = number;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const myHeroAgeSchema = z.number();"`);
    });
    it("should generate an any schema", () => {
        const source = `export type MyHeroPower = any;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const myHeroPowerSchema = z.any();"`);
    });
    it("should generate a boolean schema", () => {
        const source = `export type HavePower = boolean;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const havePowerSchema = z.boolean();"`);
    });
    it("should generate an undefined schema", () => {
        const source = `export type Nothing = undefined;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const nothingSchema = z.undefined();"`);
    });
    it("should generate a null schema", () => {
        const source = `export type MyHeroWeakness = null;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const myHeroWeaknessSchema = z.null();"`);
    });
    it("should generate a void schema", () => {
        const source = `export type MyEnemyChance = void;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const myEnemyChanceSchema = z.void();"`);
    });
    it("should generate a bigint schema", () => {
        const source = `export type loisLaneCapturedCount = bigint;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const loisLaneCapturedCountSchema = z.bigint();"`);
    });
    it("should generate a date schema", () => {
        const source = `export type lastIncidentInMetropolis = Date;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const lastIncidentInMetropolisSchema = z.date();"`);
    });
    it("should generate a literal schema (string)", () => {
        const source = `export type Kryptonite = "kryptonite";`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const kryptoniteSchema = z.literal(\\"kryptonite\\");"`);
    });
    it("should generate a literal schema (number)", () => {
        const source = `export type IdentitiesCount = 2;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const identitiesCountSchema = z.literal(2);"`);
    });
    it("should generate a literal schema (true)", () => {
        const source = `export type IsSuperman = true;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const isSupermanSchema = z.literal(true);"`);
    });
    it("should generate a literal schema (false)", () => {
        const source = `export type CanBeatZod = false;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const canBeatZodSchema = z.literal(false);"`);
    });
    ...
    it("should generate a nativeEnum schema", () => {
      ...
      ClarkKent = "clark_kent",
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const superheroSchema = z.nativeEnum(Superhero);"`);
    });
    it("should generate a never", () => {
        const source = `export type CanBeatZod = never;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const canBeatZodSchema = z.never();"`);
    });
    it("should map unknown type correctly", () => {
        const source = `export type T = unknown;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const tSchema = z.unknown();"`);
    });
    it("should generate an array schema (T[] notation)", () => {
        const source = `export type Villains = string[];`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const villainsSchema = z.array(z.string());"`);
    });
    ...
    it("should generate a tuple schema", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const lifeSchema = z.tuple([loisLaneSchema, z.array(problemSchema)]);"`);
    });
    it("should generate a tuple schema (named)", () => {
        const source = `export type Story = [subject: string, problems: string[]];`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const storySchema = z.tuple([z.string(), z.array(z.string())]);"`);
    });
    ...
    it("should generate a promise schema", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const krytonResponseSchema = z.promise(z.boolean());"`);
    });
    it("should generate a referenced schema", () => {
        const source = `export type Villain = BadGuy;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const villainSchema = badGuySchema;"`);
    });
    it("should generate a union schema", () => {
        const source = `export type Identity = "superman" | "clark kent";`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const identitySchema = z.union([z.literal(\\"superman\\"), z.literal(\\"clark kent\\")]);"`);
    });
    it("should generate a literal schema for a single union", () => {
        const source = `export type Identity = | "superman";`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const identitySchema = z.literal(\\"superman\\");"`);
    });
    ...
    it("should generate a record schema", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const enemiesPowersSchema = z.record(powerSchema);"`);
    });
    it("should generate a set schema", () => {
        const source = `export type EnemiesPowers = Set<string>;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const enemiesPowersSchema = z.set(z.string());"`);
    });
    it("should generate a function schema", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const killSupermanSchema = z.function().args(z.boolean(), z.string()).returns(z.promise(z.boolean()));"`);
    });
    it("should generate a function with optional parameter", () => {
      ...
      params?: Record<string, string | number>
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const getSupermanSkillSchema = z.function().args(z.string(), z.record(z.union([z.string(), z.number()])).optional()).returns(z.string());"`);
    });
    it("should generate a function schema (with `any` fallback on param)", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const killSupermanSchema = z.function().args(z.boolean(), z.any()).returns(z.promise(z.boolean()));"`);
    });
    it("should throw on non string record", () => {
        const source = `export type UnsupportedType = Record<number, number>;`;
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"Record<number, …> are not supported (https://github.com/colinhacks/zod/tree/v3#records)"`);
    });
    it("should throw on not supported key in omit", () => {
        const source = `export type UnsupportedType = Omit<Superman, Krytonite>;`;
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"Omit<T, K> unknown syntax: (TypeReference as K not supported)"`);
    });
    it("should throw on not supported interface with extends and index signature", () => {
     ...
     [key: string]: any;
        ...
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"interface with \`extends\` and index signature are not supported!"`);
    });
    it("should throw on not supported key in omit (union)", () => {
        ...
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"Omit<T, K> unknown syntax: (TypeReference as K union part not supported)"`);
    });
    it("should throw on not supported key in pick", () => {
        const source = `export type UnsupportedType = Pick<Superman, Krytonite>;`;
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"Pick<T, K> unknown syntax: (TypeReference as K not supported)"`);
    });
    it("should throw on not supported key in pick (union)", () => {
        ...
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"Pick<T, K> unknown syntax: (TypeReference as K union part not supported)"`);
    });
    it("should fallback on the original type for Readonly<T>", () => {
        const source = `export type ReadonlySuperman = Readonly<Superman>;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const readonlySupermanSchema = supermanSchema;"`);
    });
    it("should fallback on Array for ReadonlyArray<T>", () => {
        const source = `export type ReadonlySupermen = ReadonlyArray<Superman>;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const readonlySupermenSchema = z.array(supermanSchema);"`);
    });
    it("should generate a partial schema", () => {
        const source = `export type SupermanUnderKryptonite = Partial<Hero>;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const supermanUnderKryptoniteSchema = heroSchema.partial();"`);
    });
    it("should generate a required schema", () => {
        const source = `export type IDidFindYou = Required<VillainLocation>;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const iDidFindYouSchema = villainLocationSchema.required();"`);
    });
    it("should generate a schema with omit", () => {
        const source = `export type InvincibleSuperman = Omit<Superman, "weakness">;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const invincibleSupermanSchema = supermanSchema.omit({ \\"weakness\\": true });"`);
    });
    it("should generate a schema with omit (multiple keys)", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const veryInvincibleSupermanSchema = supermanSchema.omit({ \\"weakness\\": true, \\"wife\\": true });"`);
    });
    it("should generate a schema with pick", () => {
        const source = `export type YouJustKnowMyName = Pick<SecretIdentity, "name">;`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const youJustKnowMyNameSchema = secretIdentitySchema.pick({ \\"name\\": true });"`);
    });
    it("should generate a schema with pick (multiple keys)", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const youKnowTooMuchSchema = secretIdentitySchema.pick({ \\"name\\": true, \\"location\\": true });"`);
    });
    ...
    it("should deal with index access type (1st level)", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const supermanNameSchema = supermanSchema.shape.name;"`);
    });
    it("should deal with index access type (nested level)", () => {
        const source = `export type SupermanFlyPower = Superman["power"]["fly"]`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const supermanFlyPowerSchema = supermanSchema.shape.power.shape.fly;"`);
    });
    ...
    export type Superman = {
      powers: Array<Power>
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const supermanPowerSchema = supermanSchema.shape.powers.element;"`);
    });
    ...
    export type Superman = {
      powers: Record<string, Power>
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const supermanPowerSchema = supermanSchema.shape.powers.valueSchema;"`);
    });
    ...
    export type Superman = {
      powers: ["fly", "burnStuff"]
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const supermanPowerSchema = supermanSchema.shape.powers.items[1];"`);
    });
    // TODO
    ...
    export type Superman = {
      powers: Power[][]
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const supermanPowerSchema = supermanSchema.shape.powers.element.element;"`);
    });
    ...
    it("should deal with parenthesized type", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const secretVillainSchema = z.union([normalGuySchema, villainSchema]);"`);
    });
    it("should deal with index signature", () => {
        const source = `export type Movies = {[title: string]: Movie};`;
        expect(generate(source)).toMatchInlineSnapshot(`"export const moviesSchema = z.record(movieSchema);"`);
    });
    ...
    it("should generate an empty object schema", () => {
        ...
        expect(generate(source)).toMatchInlineSnapshot(`"export const emptySchema = z.object({});"`);
    });
    ...
    */
    ...
    it("should throw on generics", () => {
     ...
     powers: TPower[]
        ...
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"Interface with generics are not supported!"`);
    });
    ...
    it("should throw on type with generics", () => {
        ...
        expect(() => generate(source)).toThrowErrorMatchingInlineSnapshot(`"Type with generics are not supported!"`);
    });
    it("should be able to override the zod import value", () => {
        const source = `export type TheLastTest = true`;
        expect(generate(source, "zod")).toMatchInlineSnapshot(`"export const theLastTestSchema = zod.literal(true);"`);
    });
 ...
 */
function generate(sourceText, z, skipParseJSDoc) {
    const sourceFile = typescript_1.default.createSourceFile("index.ts", sourceText, typescript_1.default.ScriptTarget.Latest);
    const declaration = (0, findNode_1.findNode)(sourceFile, (node) => typescript_1.default.isInterfaceDeclaration(node) ||
    ...
    const zodConstName = `${(0, case_1.camel)(interfaceName)}Schema`;
    ...
    return typescript_1.default
        ...
        .printNode(typescript_1.default.EmitHint.Unspecified, zodSchema.statement, sourceFile);
}
//# sourceMappingURL=generateZodSchema.test.js.map